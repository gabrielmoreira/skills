...
export function redact(value, roots) {
  ...
  // A replay redacts recordings made by a different process, so the workspace
  // root built for this run never matches the one written into the file: the
  // directory is named after the pid that made it. Five paths reached a
  // baseline that way, past a literal root that was correct for the live run
  // and wrong for the replay of it.
  //
  // The name is generated by this script, so its shape is known and can be
  // collapsed by pattern where the literal missed. Bounded to not cross a quote
  // so it stays safe on the JSON path.
  s = s.replace(/[^"\s]*skill-evals-\d+/g, "<workspace>");
...
async function main(modelOverride) {
  ...
  //
  // The home directory is here because the agent leaves the workspace. Three
  // scenarios are about the agent directory itself and one is about the
  // learnings directory, so reading them is the scenario rather than a defect,
  // and the paths came back into the record naming a machine and a person. A
  // baseline written from that run carried 39 of them, including the filename
  // of a real note. The commit gate refused it, which is the gate working and
  // the wrong place to fix it.
  //
  // Order matters and home goes last: this repository lives under the home
  // directory, so redacting home first would turn <skills> into a home path.
  const roots = paths
    ? [
        [paths.conf.replace(/\\/g, "/"), "<harness>"],
        [paths.base.replace(/\\/g, "/"), "<workspace>"],
        [ROOT.replace(/\\/g, "/"), "<skills>"],
        [homedir().replace(/\\/g, "/"), "<home>"],
      ]
    : [];
