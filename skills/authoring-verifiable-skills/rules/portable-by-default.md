...
Do:
- **Name the job, not the tool.** "The project's declared test command" travels. A specific runner's name does not.
- **Name the artifact, not its location.** "The repository's configured tracker" travels. A URL does not.
...
Avoid:
- **A user directory, a home path, or a drive letter.** These identify one machine and are worthless on any other.
- **An embedded URL in prose.** A loopback or documentation host is fine. Anything else is a coupling.
- **A company or product name in an instruction.** In an example it illustrates. In an instruction it is a dependency.
- **A personal handle**, which dates the file and names someone who may not be reachable.
- **Assuming a package manager.** The repository already declares which one it uses.

Exceptions:
- **An example MAY name a real provider.** A rule about containing a payment provider's vocabulary needs that provider to be worth reading.
...
| an absolute path to a config | the config the repository already declares |
...
- **Ask whether the rule still works on a machine you have never seen.** Where the answer needs a name, the name has become a dependency.
